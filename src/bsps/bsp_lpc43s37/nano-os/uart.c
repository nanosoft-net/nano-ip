...
/** \brief Mutex for transmit (Nano-OS console vs Nano-IP log) */
static nano_os_mutex_t uart_tx_mutex;
...
void UART_Init(void)
{
    ...
    /* Create Tx mutex */
    (void)NANO_OS_MUTEX_Create(&uart_tx_mutex, QT_PRIORITY);
...
void UART_Send(const uint8_t* data, uint32_t data_len)
{
    ...
    (void)NANO_OS_MUTEX_Lock(&uart_tx_mutex, 0xFFFFFFFFu);
    
    do
    {
        /* Disable Rx and Tx interrupts */
        LPC_USART0->IER &= ~( (1u << 0u) | (1u << 1u) );

        /* If ring buffer is empty, send first byte to start transmission */
        if (UART_RingBufferIsEmpty(&tx_ring_buffer) &&
            ((LPC_USART0->LSR & (1u << 5u)) != 0) /* THRE bit */)
        {
            LPC_USART0->THR = (*data);
            data++;
            data_len--;
    ...
    (void)NANO_OS_MUTEX_Unlock(&uart_tx_mutex);
...
void UART_Receive(uint8_t* data, uint32_t data_len)
{
    ...
    {
        /* Save interrupt mask */
        uint32_t int_mask = LPC_USART0->IER;
...
void USART0_Handler()
{
    ...
    while ((LPC_USART0->LSR & ( 1u << 0u)) != 0) /* RDR bit */
    {
    ...
    /* Transmit data */
    if ((LPC_USART0->LSR & (1u << 5u)) != 0) /* THRE bit */
    {
        if (!UART_RingBufferIsEmpty(&tx_ring_buffer))
        {
            /* Read byte */

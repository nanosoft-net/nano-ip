...
static fp_synopsys_emac_interrupt_handler_t s_synopsys_emac_interrupt_handler;

/** \brief SYNOPSYS EMAC driver data */
static void* s_synopsys_emac_driver_data;
                             ...
                             uint8_t* const task_priority, uint32_t* const task_stack_size)
{
    const nano_ip_synopsys_emac_config_t config = {
                                                    LPC_ETHERNET_BASE, /* Base address of SYNOPSYS EMAC register */
    ...
    const nano_ip_error_t err = NANO_IP_SYNOPSYS_EMAC_Init(net_if, &s_synopsys_emac_interrupt_handler, &s_synopsys_emac_driver_data, &config);
    (*name) = "synopsys_emac0";
